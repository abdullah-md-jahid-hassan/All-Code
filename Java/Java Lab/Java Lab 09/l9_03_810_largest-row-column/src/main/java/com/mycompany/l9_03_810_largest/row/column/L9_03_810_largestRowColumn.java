package com.mycompany.l9_03_810_largest.row.column;


public class L9_03_810_largestRowColumn {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < matrix.length; i++) for (int j = 0; j < matrix[i].length; j++) matrix[i][j] = (int)(Math.random() * 2);
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) System.out.print(matrix[i][j]);
            System.out.println();
        }
        System.out.println("The largest row index: " + largestRow(matrix)+"\nThe largest row column: " + largestColumn(matrix));
